...
-- -- DISABLE SEF_UPDATES SINCE WE'RE DOING IT ALL NOOB-LIKE
...
-- ALTER TABLE mars_movies ADD COLUMN (id INTEGER AUTO_INCREMENT PRIMARY KEY);
...
-- ALTER TABLE mars_movies ADD FOREIGN KEY (member_id) REFERENCES mars_members(id);
...
SELECT * from mars_members WHERE id IN(
SELECT member_id from mars_salutations WHERE salutation='Mr'
);